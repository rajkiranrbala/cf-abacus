...
const arequest = retry(batch(breaker(request, {
  name: 'request_account_plugin'
})));

const prequest = retry(batch(breaker(request, {
  name: 'request_provisioning_plugin'
})));
...
const id = (oid, rtype, ppid, time, auth, cb) => {
  ...
  return arequest.get(uris.account +
...
const plan = (mpid, auth, cb) => {
  lock(mpid, (err, unlock) => {
    ...
    return prequest.get(uris.provisioning +
