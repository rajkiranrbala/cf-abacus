...
const sinkrequest = (host) => {
  if(!sinkrequests[host])  
    sinkrequests[host] = yieldable(retry(batch(throttle(breaker(request, {
      name: host
    }), throttlelimit), batchsize), retries));
  return sinkrequests[host];
};
...
const postOutput = function *(odoc, skey, stime,
  ...
  try {
    const res = yield sinkrequest(phost).post(phost + spost,
