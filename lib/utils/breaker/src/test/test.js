...
describe('abacus-breaker', () => {
  ...
  it('options object with names for breaker', () => {
    const cb = spy();
    const err = new Error('div4');
    const div4 = function div4(x, y, cb) {
      return y ? cb(undefined, x / y) : cb(err, undefined);
    };
    const b1 = breaker(div4, {
      threshold: 3,
      errors: 50,
      name: 'b1'
    });
    const b2 = breaker(div4, {
      threshold: 3,
      errors: 50,
      name: 'b2'
    });
    ...
    // then be able to get through again once, then fail fast
    clock.tick(5500);
    expect(cb.args.length).to.equal(8);
    const cerr = new breaker.CircuitBreakerError('b1.div4');
    expect(cb.args).to.deep.equal([
      [err, undefined],
      [err, undefined],
      [err, undefined],
