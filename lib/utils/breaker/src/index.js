...
const breakerfn = (fn, opt) => {
  // Convert to a function with callback
  const fcb = yieldable.functioncb(fn);

  // Preserve the original function name
  const fname = fcb.fname || fcb.name;
  
  // Breaker name
  const bname = opt.name ? [opt.name, fname].join('.') : fname;
  ...
  const wrapper = extend(function() {
    ...
    if(circuit === 'closed') {
      ...
      const h = last(perf.stats(bname, time).health);
      ...
      if(calls < opt.threshold || errors < opt.errors)
        return call(bname, fcb, initial(arguments), time, opt, circuit, cb);
    ...
    if(circuit === 'open' && time - optime > opt.reset) {
      ...
      debug('Trying one test call to function %s', bname);
      circuit = 'half-open';
      return call(
        bname, fcb, initial(arguments), time, opt, circuit, (err, val) => {

          // Close the circuit if the call succeeds, re-open it on error
          if(!err) {
            // Reset the function call reliability stats
            perf.reset(bname, time);
            circuit = 'closed';
          }
          else {
            circuit = 'open';
    ...
    debug('Function %s failed fast, circuit is %s', bname, circuit);
    return callback(bname,
...
const breaker = function(fn, opts) {
  let opt;
  if(opts instanceof Object)
    opt = extend({}, opts, options(opts.timeout, opts.threshold, opts.errors,
      opts.reset));
  else 
    opt = options(arguments[1], arguments[2], arguments[3], arguments[4]);
        
  return extend(isFunction(fn) ? breakerfn(fn, opt) : {}, 
    object(pairs(fn)), object(map(functions(fn), 
      (k) => [k, breakerfn(nbind(fn, k), opt)])));
};
